...
_POSE_LIBRARY: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    "directed": {
        "stance_logic": (
            "intentional fashion stance with clear line control",
            "directed pose with graphic body placement",
            "composed fashion-forward stance with deliberate structure",
        ),
        "weight_shift": (
            "purposeful weight shift that elongates the line",
            "controlled off-center balance with fashion-directed tension",
            "stable supporting leg with deliberate line extension",
        ),
        "arm_logic": (
            "one arm slightly set away from the body to protect silhouette definition",
            "hands placed with deliberate restraint to preserve garment structure",
            "one arm relaxed while the other creates controlled negative space near the torso",
        ),
        "torso_orientation": (
            "angled torso with graphic clarity",
            "slight body turn that sharpens the silhouette line",
            "directed shoulder angle with deliberate front readability",
        ),
        "head_direction": (
            "intentional near-camera head direction",
            "slightly lifted chin with controlled fashion attention",
            "deliberate head placement reinforcing the pose line",
        ),
        "gesture_intention": (
            "fashion-aware intention",
            "directed image control",
            "editorial-commercial precision",
        ),
    },
}
