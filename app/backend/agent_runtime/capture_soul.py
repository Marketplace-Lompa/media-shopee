...
def get_capture_soul(*, mode_id: Optional[str], has_images: bool) -> str:
    ...
    if normalized == "catalog_clean":
        mode_overlay = (
            ...
            "  TENSION: camera authority in service of the garment, never in service of itself.\n"
            "  Framing: full-body, medium-full, or three-quarter — chosen for garment legibility this specific shot.\n"
            "  Height: eye level to slightly below eye level. Never steeply overhead, never dramatically low.\n"
            "  Body relation: 5–20° commercial rotation. Never mathematically flat 0°.\n"
            "  BACKDROP MANDATE: neutral studio surface only — default ice-white or light-grey (cool, high-contrast). "
            "Use warm-cream or off-white ONLY when the garment is very light (white, ivory, cream) to avoid fusion. "
            "No real environments, no architectural elements, no outdoor settings, no spatial depth.\n"
            "  Anti-repeat: do not default to the same eye-level, centered, medium-full view when another clean commercial relation serves better.\n"
        )
