...
def get_capture_soul(*, mode_id: Optional[str], has_images: bool) -> str:
    ...
    if normalized == "catalog_clean":
        mode_overlay = (
            ...
            "  The camera disappears behind garment readability — no expressive angle,\n"
            "  no environmental storytelling, no participatory energy.\n"
            "  Default framing: medium or medium-full body — enough to show the full garment\n"
            "  silhouette, length, and hem behavior. Avoid tight portrait crops.\n"
            "  Camera height: eye level to slightly below eye level. Never steeply overhead,\n"
            "  never dramatically low.\n"
            "  Body rotation: 5–15° near-frontal. Never mathematically flat 0°, but keep rotation\n"
            "  subtle and commercially legible.\n"
            "  BACKDROP MANDATE: the background is a neutral studio surface only — default to\n"
            "  ice-white or light-grey (cool, clean, high-contrast). Use warm-cream or off-white\n"
            "  ONLY when the garment itself is very light in tone (white, ivory, off-white, cream)\n"
            "  to avoid backdrop-garment fusion. For all other garment colors, ice-white or\n"
            "  light-grey is mandatory. No real environments, no architectural elements, no outdoor\n"
            "  settings, no spatial depth that reads as a real place.\n"
            "  The backdrop exists only to not compete with the garment — it must create contrast.\n"
            "  The frame should feel clean, controlled, and product-centered — never observational,\n"
            "  never participatory, never art-directed.\n"
        )
    ...
    return (
        ...
        "  2b. DEFAULT ANGLE: use 15–30° body rotation (3/4 angle). NEVER use 0° pure frontal — it is the #1 marker of AI-generated images. Catalog_clean may use near-frontal (5-10°) but never mathematically flat.\n"
        ...
        "  - NEVER use 0° pure frontal angle — always include visible body rotation (minimum 5°, default 15-30°).\n"
